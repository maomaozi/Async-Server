...
class async_event_processor_tcp
{
  // keep a event loop
...
public:
    async_event_processor_tcp(std::string ip, int port) : psocket_fd(nullptr, [](int*){}), pepoll_fd(nullptr, [](int*){})
    {
    ...
    void event_loop()
    {
        ...
        std::thread t = std::thread(std::mem_fn(&async_event_processor_tcp::connection_resource_gc), this);
        t.detach();

        dbg_printf("%s\n", DEBUG_SERVER_CREATE_DISPATCH_THREAD);
        for(size_t i = 0; i < THREAD_NUM;++i)
        {
            std::thread t = std::thread(std::mem_fn(&async_event_processor_tcp::dispatch_worker), this, i);
            t.detach();
        }
    ...
    void serve_forever(){
        std::thread t = std::thread(std::mem_fn(&async_event_processor_tcp::event_loop), this);
        t.detach();
    }
...
public:
    ...
    void asyn_write(pdata_package_t &package, bool pass_data_by_ref=false, bool close_connection=false, std::function<void(char *)> ref_deleter=[](char *){;})
    {
        ...
        // auto deleter for ref mem
        package->deleter = ref_deleter;
        ...
        else
        {
            ...
            if(package->by_ref && package->deleter)
            {
                package->deleter(package->ref_data);
            }
    ...
    void release_package(pdata_package_t package)
    {
        p_pool[package->owner].put(package);
    }
...
private:
    ...
    void accept_handler(int thread_id, int fd)
    {
        ...
        pconnection_info->connection_id = (__uint64_t)(ntohl(client_addr.sin_addr.s_addr) << 4) | pconnection_info->port;
...
#else
        data_package_queue[package->connect->connection_id % THREAD_NUM].enqueue(package);
#endif
        }
        else if(data_size == 0)
        {
    ...
    void write_handler(int thread_id, epoll_event *event)
    {
        ...
        if(!package_list.empty())
        {
            ...
            if (sent_length >= 0)
            {
                ...
                {
                    // all data is sent
                    // recycle package, and call deleter for ref
                    if(package->by_ref && package->deleter)
                    {
                        package->deleter(package->ref_data);
                    }
